...
export default function VocabDrill({ vocab, lessonId }) {
    ...
    const toggleCover = (word, field) => {
        setCoverStates(prev => {
            const current = prev[word.spanish]?.[field] ?? false;
            const newValue = !current;
            // 如果新值是 false（即掀开贴纸，显示内容），则朗读西语单词
            if (newValue === false) {
                speak(word.spanish);
            }
            return {
                                        ...
                                        {cover.spanishCovered ? (
                                            ...
                                            <>
                                                <div className="flex items-center justify-between mb-1">
                                                    <div className="text-xs text-slate-500">西语</div>
                                                    <button
                                                        onClick={(e) => {
                                                            e.stopPropagation();
                                                            speak(word.spanish);
                                                        }}
                                                        className="p-1 text-slate-500 hover:text-spanish-red rounded-full hover:bg-slate-100"
                                                        title="播放发音"
                                                    >
                                                        <Volume2 size={14} />
                                                    </button>
