...
import { BookOpen, GraduationCap, ArrowLeft, CheckCircle2, Brain, Languages, BookText, ListChecks, Settings, X } from 'lucide-react';
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-spanish-red to-orange-600">
              {activeLesson ? `L${activeLesson.id}: ${activeLesson.title}` : 'Spanish Cram Suite'}
            </h1>
            {activeLesson && (
        ...
        {!activeLessonId ? (
                ...
                {courseData.map((lesson) => (
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      <div className="text-xs font-bold text-spanish-red uppercase tracking-wider mb-1">Lección {lesson.id}</div>
          ...
          <div className="h-full flex flex-col max-w-4xl mx-auto w-full bg-white shadow-xl border-x border-slate-200">
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50 p-6">
              {activeTab === 'vocab' && activeLesson.vocab && <VocabDrill key={activeLesson.id} vocab={activeLesson.vocab} lessonId={activeLesson.id} />}
              {activeTab === 'grammar' && activeLesson.grammar && <GrammarQuiz key={activeLesson.id} data={activeLesson.grammar} vocab={activeLesson.vocab} lessonId={activeLesson.id} />}
              {activeTab === 'reading' && activeLesson.reading && <ReadingComp key={activeLesson.id} data={activeLesson.reading} vocab={activeLesson.vocab} grammarTitle={activeLesson.grammar?.title || ''} lessonId={activeLesson.id} />}
              {activeTab === 'translation' && activeLesson.translation && <TranslationTrainer key={activeLesson.id} data={activeLesson.translation} vocab={activeLesson.vocab} lessonId={activeLesson.id} />}
              {activeTab === 'verbs' && activeLesson.verbs && <VerbConjugation key={activeLesson.id} verbs={activeLesson.verbs} lessonId={activeLesson.id} />}
            </div>
          </div>
        )}
      </main>
