...
import { Brain, Check, X, Loader2, ArrowRight, RefreshCw, HelpCircle, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function GrammarQuiz({ data, vocab, lessonId }) {
    ...
    // Initial load - use static data first
    useEffect(() => {
        if (data && data.quiz) {
    ...
    const generateNewQuestions = async () => {
        ...
        setError(null);
        try {
            const newQuestions = await generateGrammarQuiz(data, vocab, 5);
    ...
    if (currentIndex >= questions.length - 1 && showResult) {
        return (
            <div className="max-w-2xl mx-auto mt-8 bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-center">
                <div className="w-20 h-20 bg-yellow-100 text-spanish-yellow rounded-full flex items-center justify-center mx-auto mb-4">
                    <Brain size={40} />
                </div>
        ...
        <div className="max-w-2xl mx-auto mt-8">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                    <Brain className="text-spanish-red" />
                    语法挑战
                </h2>
