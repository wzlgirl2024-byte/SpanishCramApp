...
2. **适当扩展**：可以少量使用"已学词汇"和"已学语法"来丰富句子，但不要喧宾夺主。
3. **绝对禁止**：不要使用学生完全没学过的生僻词！
4. 句子要简短、自然，适合初学者。
5. 每次生成都要有所不同，不要重复旧题。
...
- 100分：完全正确或意思完全一致
- 85-99分：意思正确，但有细微瑕疵（如拼写、标点、冠词、单复数），或者使用了不同的表达方式但也是对的。
- 60-84分：主要意思对，但有明显的语法错误或用词不当。
- 1-59分：意思错误或严重语法错误。
- 0分：完全错误或无关

请注意：如果学生使用了同义词或不同的句式，只要意思对，请给高分。不要太死板。
...
只返回JSON，不要其他文字。`;
        ...
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        content: '你是一位鼓励型、乐于助人的西班牙语教师。评判时请注重意思表达，不要过于纠结细微的语法差异。'
                    },
                    {
                        role: 'user',
                        content: prompt
